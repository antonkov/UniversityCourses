...
hold on;
semilogy(xsIndB, pebsBSC, 'DisplayName', 'BSC with all terms of F(D)');
hold on;
semilogy(xsIndB, pebsBSC10, 'DisplayName','BSC with 10 terms of F(D)');
hold on;
semilogy(xsIndB, pebsBSC5, 'DisplayName', 'BSC with 5 terms of F(D)');
legend('show')
title(names(testId,:));
xlabel('E_b/No dB');
ylabel('P_{eb}');
